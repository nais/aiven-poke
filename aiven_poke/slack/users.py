...
USERS_HEADER = (
    "*Found service users defined in `{cluster_name}`, for the `{main_project}` pool, with expiring credentials*"
)
SOLUTION_HEADER = "*Solution*"
SOLUTION_PROTECTED = " ".join(
    textwrap.dedent(f"""\
...
def create_user_payload(team, channel, users: list[ExpiringUser], main_project, cluster_name):
    ...
    fallback_text = FALLBACK.format(
        main_project=main_project, usernames="\n* ".join(usernames), cluster_name=cluster_name
    )
    ...
    return Payload(channel, attachments=[Attachment(Color.WARNING, fallback_text, blocks=blocks)])
